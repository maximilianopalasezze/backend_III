...
@SpringBootTest(webEnvironment=SpringBootTest.WebEnvironment.MOCK, properties={
    ...
    "spring.datasource.url=jdbc:h2:mem:seguridad_movil;MODE=MySQL;DB_CLOSE_DELAY=-1",
    "spring.datasource.driver-class-name=org.h2.Driver",
    "spring.datasource.username=sa", "spring.datasource.password=",
    "spring.sql.init.mode=always",
    "spring.sql.init.schema-locations=classpath:schema-integracion.sql"
})
class SeguridadMovilTests {
    ...
    String login(boolean consulta) throws Exception {
        String usuario=consulta?"movil-consulta":"movil-demo";
    ...
    String token(String issuer,String audience,String canal,Instant expires,String cuenta) {
        ...
        var claims=JwtClaimsSet.builder().issuer(issuer).subject("prueba")
            ...
            .claim("scope","movil:lectura").build();
    ...
    @Test void loginYConsultaConTokenFirmado() throws Exception {
        mvc.perform(get("/api/bff/movil/cuentas/106").secure(true).header("Authorization","Bearer "+login(false)))
            .andExpect(status().isOk()).andExpect(jsonPath("$.canal").value("MOVIL"))
    ...
    @Test void rechazaPasswordIncorrecta() throws Exception {
        mvc.perform(post("/api/auth/token").secure(true).contentType("application/json")
            .content("{\"usuario\":\"movil-demo\",\"password\":\"incorrecta\"}"))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaAusenciaDeTokenYAntiguaApiKey() throws Exception {
        mvc.perform(get("/api/bff/movil/cuentas/106").secure(true)).andExpect(status().isUnauthorized());
        mvc.perform(get("/api/bff/movil/cuentas/106").secure(true).header("X-BFF-API-Key","movil-local-key"))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaTokenExpirado() throws Exception {
        String t=token("urn:banco-xyz:movil","bff-movil","MOVIL",Instant.now().minusSeconds(1),"106");
        mvc.perform(get("/api/bff/movil/cuentas/106").secure(true).header("Authorization","Bearer "+t))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaFirmaAlterada() throws Exception {
        ...
        mvc.perform(get("/api/bff/movil/cuentas/106").secure(true).header("Authorization","Bearer "+String.join(".",partes)))
            .andExpect(status().isUnauthorized());
    }
    @Test void validaEmisorAudienciaYCanal() throws Exception {
        for(String t:List.of(
                token("urn:otro","bff-movil","MOVIL",Instant.now().plusSeconds(60),"106"),
                token("urn:banco-xyz:movil","bff-otro","MOVIL",Instant.now().plusSeconds(60),"106"),
                token("urn:banco-xyz:movil","bff-movil","OTRO",Instant.now().plusSeconds(60),"106"))) {
            mvc.perform(get("/api/bff/movil/cuentas/106").secure(true).header("Authorization","Bearer "+t))
                .andExpect(status().isUnauthorized());
        }
    }
    ...
    @Test void impideConsultarOtraCuenta() throws Exception {
        mvc.perform(get("/api/bff/movil/cuentas/107").secure(true).header("Authorization","Bearer "+login(false)))
            .andExpect(status().isForbidden());
    }
    ...
    @Test void noIncluyeBatchNiAplicacionesDeOtrosCanales() {
        for(String nombre:List.of("org.springframework.batch.core.job.Job", "cl.duoc.bank_batch.BankBatchApplication",
                "cl.duoc.bff.web.BffWebApplication","cl.duoc.bff.cajero.BffCajeroApplication"))
            assertThat(org.springframework.util.ClassUtils.isPresent(nombre,getClass().getClassLoader())).isFalse();
    }
    @Test void aplicaLimiteMovilConJwt() throws Exception {
        mvc.perform(get("/api/bff/movil/cuentas/106/movimientos?limite=11").secure(true)
            .header("Authorization","Bearer "+login(false))).andExpect(status().isBadRequest());
    }
}
