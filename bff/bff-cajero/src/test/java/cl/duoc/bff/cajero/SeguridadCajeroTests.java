...
@SpringBootTest(webEnvironment=SpringBootTest.WebEnvironment.MOCK, properties={
    ...
    "spring.datasource.url=jdbc:h2:mem:seguridad_cajero;MODE=MySQL;DB_CLOSE_DELAY=-1",
    "spring.datasource.driver-class-name=org.h2.Driver",
    "spring.datasource.username=sa", "spring.datasource.password=",
    "spring.sql.init.mode=always",
    "spring.sql.init.schema-locations=classpath:schema-integracion.sql"
})
class SeguridadCajeroTests {
    ...
    String login(boolean consulta) throws Exception {
        String usuario=consulta?"cajero-consulta":"cajero-demo";
    ...
    String token(String issuer,String audience,String canal,Instant expires,String cuenta) {
        ...
        var claims=JwtClaimsSet.builder().issuer(issuer).subject("prueba")
            ...
            .claim("scope","cajero:lectura").build();
        return encoder.encode(JwtEncoderParameters.from(JwsHeader.with(MacAlgorithm.HS256).build(),claims)).getTokenValue();
    }
    @Test void loginYConsultaConTokenFirmado() throws Exception {
        mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true).header("Authorization","Bearer "+login(false)))
            .andExpect(status().isOk()).andExpect(jsonPath("$.canal").value("CAJERO"))
    ...
    @Test void rechazaPasswordIncorrecta() throws Exception {
        mvc.perform(post("/api/auth/token").secure(true).contentType("application/json")
            .content("{\"usuario\":\"cajero-demo\",\"password\":\"incorrecta\"}"))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaAusenciaDeTokenYAntiguaApiKey() throws Exception {
        mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true)).andExpect(status().isUnauthorized());
        mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true).header("X-BFF-API-Key","cajero-local-key"))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaTokenExpirado() throws Exception {
        String t=token("urn:banco-xyz:cajero","bff-cajero","CAJERO",Instant.now().minusSeconds(1),"106");
        mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true).header("Authorization","Bearer "+t))
            .andExpect(status().isUnauthorized());
    }
    @Test void rechazaFirmaAlterada() throws Exception {
        ...
        mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true).header("Authorization","Bearer "+String.join(".",partes)))
            .andExpect(status().isUnauthorized());
    }
    @Test void validaEmisorAudienciaYCanal() throws Exception {
        for(String t:List.of(
                token("urn:otro","bff-cajero","CAJERO",Instant.now().plusSeconds(60),"106"),
                token("urn:banco-xyz:cajero","bff-otro","CAJERO",Instant.now().plusSeconds(60),"106"),
                token("urn:banco-xyz:cajero","bff-cajero","OTRO",Instant.now().plusSeconds(60),"106"))) {
            mvc.perform(get("/api/bff/cajero/cuentas/106/saldo").secure(true).header("Authorization","Bearer "+t))
                .andExpect(status().isUnauthorized());
        }
    }
    ...
    @Test void impideConsultarOtraCuenta() throws Exception {
        mvc.perform(get("/api/bff/cajero/cuentas/107/saldo").secure(true).header("Authorization","Bearer "+login(false)))
            .andExpect(status().isForbidden());
    }
    ...
    @Test void noIncluyeBatchNiAplicacionesDeOtrosCanales() {
        for(String nombre:List.of("org.springframework.batch.core.job.Job", "cl.duoc.bank_batch.BankBatchApplication",
                "cl.duoc.bff.web.BffWebApplication","cl.duoc.bff.movil.BffMovilApplication"))
            assertThat(org.springframework.util.ClassUtils.isPresent(nombre,getClass().getClassLoader())).isFalse();
    }
    @Test void permisoDeLecturaNoAutorizaRetiro() throws Exception {
        mvc.perform(post("/api/bff/cajero/cuentas/106/retiros").secure(true)
            .header("Authorization","Bearer "+login(true)).contentType("application/json")
            .content("{\"monto\":2000}")).andExpect(status().isForbidden());
        assertThat(jdbc.queryForObject("SELECT saldo FROM cuentas WHERE cuenta_id=106",java.math.BigDecimal.class))
            .isEqualByComparingTo("12180.00");
    }
    @Test void retiroConPermisoActualizaSaldoYAuditoria() throws Exception {
        mvc.perform(post("/api/bff/cajero/cuentas/106/retiros").secure(true)
            .header("Authorization","Bearer "+login(false)).contentType("application/json")
            .content("{\"monto\":2000}")).andExpect(status().isOk());
        assertThat(jdbc.queryForObject("SELECT saldo FROM cuentas WHERE cuenta_id=106",java.math.BigDecimal.class))
            .isEqualByComparingTo("10180.00");
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM operaciones_cajero",Integer.class)).isEqualTo(1);
    }
}
